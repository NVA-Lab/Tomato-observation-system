...
from trackers import MotionEstimator, SORTTracker
...
def run(config: dict) -> dict:
    ...
    if use_motion:
        ...
        print("[SORT] motion: trackers.MotionEstimator (on)")
    else:
        print("[SORT] motion: off")
    ...
    while True:
        ...
        if coord is not None and not is_reasonable_motion_transform(coord, w, h):
            if bad_motion_warns < 5:
                print("[SORT][WARN] unstable motion transform -> fallback to identity")
                bad_motion_warns += 1
